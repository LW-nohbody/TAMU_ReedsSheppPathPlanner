...
public partial class VehicleAgent : Node3D
{
    ...
    private readonly HashSet<VehicleAgent> _frozenCarsAlreadyHandled = new();
        ...
        int sectorIndex, int totalSectors, double sampleStepMeters)
    {
        ...
        if (currentVehicle != null)
        {
            _lastPosition = currentVehicle.GlobalTransform.Origin;
            GD.Print($"[VehicleAgent] {this.Name} assigned to sector {SectorIndex}/{_totalSectors}");
            GD.Print($"🤖 [VehicleAgent] {this.Name} received DigConfig: DigDepth={_digConfig.DigDepth:F2}m, DigRadius={_digConfig.DigRadius:F2}m, DepthRate={_digConfig.DepthRatePerSecond:F2}m/s (config object hash: {_digConfig.GetHashCode()})");
        }
    }
    ...
    public void UpdateDigBehavior(float deltaSeconds)
    {
        ...
        {
            GD.PushError($"⚠️ [VehicleAgent] {this.Name} missing required services for digging. {nameof(_digService)}={_digService != null}, {nameof(_terrain)}={_terrain != null}, {nameof(_scheduler)}={_scheduler != null}");
            return;
        }
        ...
        switch (DigState.State)
        {
            ...
            case DigState.TaskState.TravelingToDigSite:
                ...
                //             GD.Print($"� [VehicleAgent] {this.Name} STUCK {_stuckAttempts} times at current target! Abandoning site and finding new target...");
                ...
                //             GD.Print($"🚨 [VehicleAgent] {this.Name} STUCK (attempt {_stuckAttempts}/{MaxStuckAttempts})! Backing up and replanning with offset...");
                ...
                //             currentVehicle.GlobalTransform = new Transform3D(Agent.GlobalTransform.Basis, backupTarget);
                ...
                //             GD.Print($"🔄 [VehicleAgent] {this.Name} backed up with random offset, replanning with angle variation...");
                ...
                if (distToDig < _digConfig.AtSiteThreshold)
                {
                    ...
                    if (DigState.CurrentSiteComplete)
                    {
                        ...
                        GD.Print($"🎯 [VehicleAgent] {this.Name} arrived at NEW dig site, initial height: {DigState.InitialDigHeight:F2}m, TARGET DEPTH FROM CONFIG: {_digConfig.DigDepth:F2}m");
                    }
                    else
                    {
                        // Returning to same site after dump - keep volume tracker
                        GD.Print($"🔄 [VehicleAgent] {this.Name} RETURNED to dig site after dump, already excavated: {DigState.CurrentSiteVolumeExcavated:F3}m³, continuing...");
                    }
                }
                break;
            ...
            case DigState.TaskState.Digging:
                ...
                {
                    GD.Print($"💼 [VehicleAgent] {this.Name} PAYLOAD FULL ({DigState.CurrentPayload:F2}/{DigState.MaxPayload:F2}m³), going to dump (will return to continue this site)");
                ...
                if (_timeSinceLastSiteCheck >= SiteCheckInterval)
                {
                    ...
                    GD.Print($"⛏️ [VehicleAgent] {this.Name} digging: maxH={maxHeight:F3}m, centerH={centerHeight:F3}m, floor={floorY:F3}m | volume={DigState.CurrentSiteVolumeExcavated:F3}/{cylinderVolume:F3}m³ ({volumePercent:F0}%) | payload={DigState.CurrentPayload:F2}/{DigState.MaxPayload:F2}m³");
                    ...
                    if (noMoreDirt)
                    {
                        ...
                        GD.Print($"✅ [VehicleAgent] {this.Name} SITE COMPLETE! All terrain at floor (maxH={maxHeight:F3}m ≈ floor={floorY:F3}m). Excavated {DigState.CurrentSiteVolumeExcavated:F3}m³. Moving to next site!");
            ...
            case DigState.TaskState.Dumping:
                ...
                GD.Print($"📦 [VehicleAgent] {this.Name} dumped {dumpedAmount:F2}m³ (dump #{DigState.DumpCount})");

                // Check if there's still dirt to dig globally
                if (!HasRemainingTerrain())
                {
                    DigState.State = DigState.TaskState.Complete;
                    GD.Print($"🏁 [VehicleAgent] {this.Name} ARENA COMPLETE - no dirt remaining anywhere in arena!");
                    break;
                }
                ...
                if (!DigState.CurrentSiteComplete)
                {
                    ...
                    GD.Print($"🔄 [VehicleAgent] {this.Name} returning to SAME dig site (already excavated {DigState.CurrentSiteVolumeExcavated:F3}m³)");
                ...
                else
                {
                    ...
                    GD.Print($"🎯 [VehicleAgent] {this.Name} previous site complete, finding NEW dig site");
                    DigState.State = DigState.TaskState.Idle;
                    RequestNewDigTarget();
                }
                break;
    ...
    private static System.Collections.Generic.Dictionary<VehicleAgent, CylinderObstacle> FrozenCarObstacles
        = new System.Collections.Generic.Dictionary<VehicleAgent, CylinderObstacle>();
    ...
    public void RegisterFrozenCarObstacle()
    {
        ...
        var pos = currentVehicle.GlobalTransform.Origin;
        ...
        GD.Print($"[VehicleAgent] Registered frozen obstacle for {this.Name}");
    }
    ...
    public void RemoveFrozenCarObstacle()
    {
        ...
        GD.Print($"[VehicleAgent] Removed frozen obstacle for {this.Name}");
    }
    ...
    {
        var myPos = currentVehicle.GlobalTransform.Origin;
        ...
        foreach (var kv in FrozenCarObstacles)
        {
            ...
            var otherPos = other.currentVehicle.GlobalTransform.Origin;
            ...
            if (dist < FreezeRadius)
            {
                ...
                if (!_frozenCarsAlreadyHandled.Contains(other))
                {
                    ...
                    GD.Print($"[VehicleAgent] {this.Name} replanning (new frozen car: {other.Name})");
            ...
            else if (dist > UnfreezeRadius)
            {
                ...
                if (_frozenCarsAlreadyHandled.Contains(other))
                {
                    ...
                    GD.Print($"[VehicleAgent] {this.Name} untracked frozen car {other.Name}");
                }
            }
        }
    }
    ...
    public void FreezeForPriority()
    {
        ...
        if (!_hasReplannedFromFreeze)
        {
            ...
            GD.Print($"[VehicleAgent] {this.Name} frozen — triggering ONE replan.");
        ...
        {
            GD.Print($"[VehicleAgent] {this.Name} frozen (already replanned, skipping).");
        }
    }
    ...
    public void UnfreezeFromPriority()
    {
        ...
        GD.Print($"[VehicleAgent] {this.Name} unfrozen.");
    }
    ...
    private void RequestNewDigTarget()
    {
        ...
        {
            GD.PrintErr("[VehicleAgent] Cannot request new dig target - scheduler or terrain is null");
            return;
        }
        ...
        {
            GD.Print($"🏁 [VehicleAgent] {this.Name} SECTOR {SectorIndex} COMPLETE - no dirt remaining in assigned sector");
            DigState.State = DigState.TaskState.Complete;
            return;
        }
        ...
        if (digPos.Y < 0f)
        {
            ...
            GD.Print($"🏁 [VehicleAgent] {this.Name} SECTOR {SectorIndex} COMPLETE - no valid dig targets available (all blocked or too shallow)");
            DigState.State = DigState.TaskState.Complete;
            return;
        }
        ...
        GD.Print($"[VehicleAgent] {this.Name} (sector {SectorIndex}) assigned new dig target at ({digPos.X:F1}, {digPos.Z:F1}), height {digPos.Y:F2}m");
    }
    ...
    private Vector3 FindBestDigInSector()
    {
        ...
        {
            GD.PrintErr($"[VehicleAgent] {this.Name} sector {SectorIndex}: NO VALID dig targets found! Checked {candidatesChecked} cells, {candidatesInSector} in sector (using max height in {_digConfig.DigRadius:F2}m radius)");
        ...
        GD.Print($"[VehicleAgent] {this.Name} sector {SectorIndex}: checked {candidatesChecked} cells, {candidatesInSector} in sector, best MAX height: {bestPos.Y:F2}m at ({bestPos.X:F1}, {bestPos.Z:F1}) (dig radius: {_digConfig.DigRadius:F2}m)");
    ...
    private void PlanPathToDigSite(Vector3 digPos, float approachYaw)
    {
        ...
        var robotPos = currentVehicle.GlobalTransform.Origin;
        var fwd = -currentVehicle.GlobalTransform.Basis.Z;
    ...
    {
        var position = currentVehicle.GlobalTransform.Origin;
        ...
        if (DigState.IsPayloadFull)
        {
            GD.Print($"[VehicleAgent] {this.Name} payload full, going to dump");
            DigState.State = DigState.TaskState.TravelingToDump;
            PlanPathToDump();
        }
    }
    ...
    {
        Vector3 robotPos = currentVehicle.GlobalTransform.Origin;
    ...
    private void MarkSiteAsFailed(Vector3 site)
    {
        if (!_failedDigSites.Contains(site))
        {
            ...
            GD.Print($"❌ [VehicleAgent] {this.Name} marked site ({site.X:F1}, {site.Z:F1}) as FAILED/UNREACHABLE");
        }
    }
