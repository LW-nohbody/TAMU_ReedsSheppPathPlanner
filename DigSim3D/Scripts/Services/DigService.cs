...
/// </summary>
public partial class DigService : Node
{
    public bool _simPaused = false;
    ...
    private double _timeSinceLastMeshUpdate = 0.0;
    ...
    {
        if (_terrainModifiedSinceLastUpdate && !_simPaused)
        {
            _timeSinceLastMeshUpdate += (float)delta;
            if (_timeSinceLastMeshUpdate >= MeshUpdateInterval)
            {
                _terrain.RebuildMesh();
                _terrainModifiedSinceLastUpdate = false;
                _timeSinceLastMeshUpdate = 0.0;
            }
        }
    }
