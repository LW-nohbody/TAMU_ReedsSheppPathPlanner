...
//     /// </summary>
//     public sealed class HybridDubinsPlanner : IHybridPlanner
//     {
...
//             float angleWallDiff = angleToWall - (float)goal.Theta;
...
//                 goal = Pose.Create(goal.X, goal.Y, angleToWall - yawSign * 0.48);
//             }

//             var (dPoints, dGears) = DAdapter.ComputePath3D(
//                 startPos, start.Theta,
//                 goalPos, goal.Theta,
...
//             GD.Print($"[HybridDubinsPlanner] Goal Pos: ({goalPos.X}, {goalPos.Y}) and orientation: {goal.Theta}");

//             var rsPts = dPoints.ToList();
//             if (obstacles.Count == 0 || PathIsValid(rsPts, obstacles, goal.Theta, turnRadius, world.Terrain.Radius))
//             {
...
//             goal.Theta, start.Theta, arenaRadius);
...
//                     double distToMid = Math.Sqrt(Math.Pow(mid.X - segStart.X, 2) + Math.Pow(mid.Z - segStart.Z, 2));
//                     double maxYaw = distToMid / world.Terrain.Radius;
